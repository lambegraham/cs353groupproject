...
function CRUD() {
  ...
  React.useEffect(() => {
    const fetchData = async() => {
      ...
      //^this gets all the doucments in the collection
      //const data = await db.collection("users").doc("Modules").get()
...
***REMOVED***, [])
  ...
  const onCreate=() => { //creates data to the firebase
    const db = firebase.firestore();
    db.collection('users').doc('Modules').add({moduleCode: newModuleCode, moduleName: newModuleName, moduleCA: newModuleCA, moduleExam: newModuleExam***REMOVED***);
***REMOVED***
