...
// async await/promise!
let result = await walk.async('../',{return_object:true***REMOVED***)
//result['path'] = {statObject***REMOVED***
...
```
  ...
  - walkdir.sync/walkdir.async only
	...
	```js
	{
	  "return_object": false, // if true the sync return will be in {path:stat***REMOVED*** format instead of [path,path,...]
