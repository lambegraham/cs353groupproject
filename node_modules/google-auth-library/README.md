...
```js
const {GoogleAuth***REMOVED*** = require('google-auth-library');
...
***REMOVED***);
  ...
  const url = `https://dns.googleapis.com/dns/v1/projects/${projectId***REMOVED***`;
  const res = await client.request({ url ***REMOVED***);
...
``` js
const {OAuth2Client***REMOVED*** = require('google-auth-library');
...
async function main() {
  ...
  const res = await oAuth2Client.request({url***REMOVED***);
  console.log(res.data);
  ...
  ***REMOVED***);
      ...
      .createServer(async (req, res) => {
        try {
          if (req.url.indexOf('/oauth2callback') > -1) {
            ...
            console.log(`Code is ${code***REMOVED***`);
    ...
    ***REMOVED***)
      .listen(3000, () => {
        ...
        open(authorizeUrl, {wait: false***REMOVED***).then(cp => cp.unref());
    ***REMOVED***);
    destroyer(server);
***REMOVED***);
***REMOVED***
...
``` js
const {JWT***REMOVED*** = require('google-auth-library');
...
async function main() {
  ...
  const url = `https://dns.googleapis.com/dns/v1/projects/${keys.project_id***REMOVED***`;
  const res = await client.request({url***REMOVED***);
  console.log(res.data);
***REMOVED***
...
```js
const {auth***REMOVED*** = require('google-auth-library');
...
``` js
const {auth, Compute***REMOVED*** = require('google-auth-library');
