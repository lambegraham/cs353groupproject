 ...
 * @param {function(?Error, string=)***REMOVED*** [callback] Optional completion callback
 * @returns {number|undefined***REMOVED*** Exit code, if known
 */
exports.main = function main(args, callback) {
  ...
  ***REMOVED***);
    ...
    Object.keys(argv).forEach(function(key) {
        var camelKey = key.replace(/-([a-z])/g, function($0, $1) { return $1.toUpperCase(); ***REMOVED***);
        if (camelKey !== key)
  ...
  ***REMOVED***);
    ...
    if (!files.length) {
        var descs = Object.keys(targets).filter(function(key) { return !targets[key].private; ***REMOVED***).map(function(key) {
    ...
    function callTarget() {
        target(root, argv, function targetCallback(err, output) {
          ...
          ***REMOVED***
            try {
                if (argv.out)
                    fs.writeFileSync(argv.out, output, { encoding: "utf8" ***REMOVED***);
                else if (!callback)
