 ...
 * [set()]{@link DocumentReference#set***REMOVED***) consisting of fields mapped
 * to values.
 *
 * @typedef {Object.<string, *>***REMOVED*** DocumentData
 */
/**
 * Update data (for use with [update]{@link DocumentReference#update***REMOVED***)
 ...
 *
 * @typedef {Object.<string, *>***REMOVED*** UpdateData
 */
/**
 * An options object that configures conditional behavior of
 * [update()]{@link DocumentReference#update***REMOVED*** and
 * [delete()]{@link DocumentReference#delete***REMOVED*** calls in
 * [DocumentReference]{@link DocumentReference***REMOVED***,
 * [WriteBatch]{@link WriteBatch***REMOVED***, and
 * [Transaction]{@link Transaction***REMOVED***. Using Preconditions, these calls
 ...
 *
 *   console.log(`Deleting document at update time: ${updateTime.toDate()***REMOVED***`);
 *   return documentRef.delete({ lastUpdateTime: updateTime ***REMOVED***);
 * ***REMOVED***);
 *
 * @property {string***REMOVED*** lastUpdateTime The update time to enforce (specified as
 * an ISO 8601 string).
 * @typedef {Object***REMOVED*** Precondition
 */
/**
 * An options object that configures the behavior of
 * [set()]{@link DocumentReference#set***REMOVED*** calls in
 * [DocumentReference]{@link DocumentReference***REMOVED***,
 * [WriteBatch]{@link WriteBatch***REMOVED***, and
 * [Transaction]{@link Transaction***REMOVED***. These calls can be
 * configured to perform granular merges instead of overwriting the target
 * documents in their entirety by providing a SetOptions object with
 * { merge : true ***REMOVED***.
 *
 * @property {boolean***REMOVED*** merge Changes the behavior of a set() call to only
 * replace the values specified in its data argument. Fields omitted from the
 * set() call remain untouched.
 * @property {Array<(string|FieldPath)>***REMOVED*** mergeFields Changes the behavior of
 ...
 * @typedef {Object***REMOVED*** SetOptions
 */
/**
 * An options object that can be used to configure the behavior of
 * [getAll()]{@link Firestore#getAll***REMOVED*** calls. By providing a `fieldMask`, these
 * calls can be configured to only return a subset of fields.
 *
 * @property {Array<(string|FieldPath)>***REMOVED*** fieldMask Specifies the set of fields
 ...
 * @typedef {Object***REMOVED*** ReadOptions
 */
/**
 * The Firestore client represents a Firestore Database and is the entry point
 * for all Firestore operations.
 *
 * @see [Firestore Documentation]{@link https://firebase.google.com/docs/firestore/***REMOVED***
 *
 * @class
 *
...
class Firestore {
    /**
     * @param {Object=***REMOVED*** settings [Configuration object](#/docs).
     * @param {string=***REMOVED*** settings.projectId The project ID from the Google
     ...
     * Credentials***REMOVED***
     * @param {string=***REMOVED*** settings.keyFilename Local file containing the Service
     * Account credentials as downloaded from the Google Developers Console. Can
     * be omitted in environments that support
     * {@link https://cloud.google.com/docs/authentication Application Default
     * Credentials***REMOVED***. To configure Firestore with custom credentials, use
     * `settings.credentials` and provide the `client_email` and `private_key` of
     * your service account.
     * @param {{client_email:string=, private_key:string=***REMOVED***=***REMOVED*** settings.credentials
     * The `client_email` and `private_key` properties of the service account
     * to use with your Firestore project. Can be omitted in environments that
     * support {@link https://cloud.google.com/docs/authentication Application
     * Default Credentials***REMOVED***. If your credentials are stored in a JSON file, you
     * can specify a `keyFilename` instead.
     * @param {string=***REMOVED*** settings.host The host to connect to.
     * @param {boolean=***REMOVED*** settings.ssl Whether to use SSL when connecting.
     * @param {boolean=***REMOVED*** settings.timestampsInSnapshots Specifies whether to use
    ...
    constructor(settings) {
      ...
      ***REMOVED***
        if (process.env.FIRESTORE_EMULATOR_HOST) {
            ...
            const emulatorSettings = Object.assign(Object.assign(Object.assign({***REMOVED***, settings), libraryHeader), { host: process.env.FIRESTORE_EMULATOR_HOST, ssl: false ***REMOVED***);
            ...
            emulatorSettings.customHeaders = Object.assign({***REMOVED***, emulatorSettings.customHeaders, { Authorization: 'Bearer owner' ***REMOVED***);
            this.validateAndApplySettings(emulatorSettings);
      ***REMOVED***
        else {
            this.validateAndApplySettings(Object.assign(Object.assign({***REMOVED***, settings), libraryHeader));
      ***REMOVED***
      ...
      ***REMOVED***
        this._clientPool = new pool_1.ClientPool(MAX_CONCURRENT_REQUESTS_PER_CLIENT, () => {
            ...
            if (this._settings.ssl === false) {
                ...
                client = new module.exports.v1(Object.assign({ sslCreds ***REMOVED***, this._settings));
          ***REMOVED***
            else {
  ...
  ***REMOVED***
     ...
     *
     * @param {object***REMOVED*** settings The settings to use for all Firestore operations.
     */
    settings(settings) {
        validate_1.validateObject('settings', settings);
        validate_1.validateString('settings.projectId', settings.projectId, { optional: true ***REMOVED***);
        validate_1.validateBoolean('settings.timestampsInSnapshots', 
        // tslint:disable-next-line deprecation
        settings.timestampsInSnapshots, { optional: true ***REMOVED***);
        if (this._settingsFrozen) {
            throw new Error('Firestore has already been initialized. You can only call ' +
                ...
                'Firestore object.');
      ***REMOVED***
        const mergedSettings = Object.assign(Object.assign({***REMOVED***, this._settings), settings);
    ...
    validateAndApplySettings(settings) {
      ...
      ***REMOVED***
        if (settings.host !== undefined) {
          ...
          ***REMOVED***
            const url = new url_1.URL(`http://${settings.host***REMOVED***`);
  ...
  ***REMOVED***
     ...
     */
    get formattedName() {
        return `projects/${this.projectId***REMOVED***/databases/${path_1.DEFAULT_DATABASE_ID***REMOVED***`;
  ***REMOVED***
    /**
     ...
     * @returns {DocumentReference***REMOVED*** The
     * [DocumentReference]{@link DocumentReference***REMOVED*** instance.
     *
     * @example
     * let documentRef = firestore.doc('collection/document');
     * console.log(`Path of document is ${documentRef.path***REMOVED***`);
     */
    doc(documentPath) {
  ...
  ***REMOVED***
     ...
     * @returns {CollectionReference***REMOVED*** The
     * [CollectionReference]{@link CollectionReference***REMOVED*** instance.
     *
     * @example
     * let collectionRef = firestore.collection('collection');
     *
     * // Add a document with an auto-generated ID.
     * collectionRef.add({foo: 'bar'***REMOVED***).then((documentRef) => {
     *   console.log(`Added document at ${documentRef.path***REMOVED***)`);
     * ***REMOVED***);
     */
  ...
  ***REMOVED***
     ...
     * given collectionId.
     *
     * @param {string***REMOVED*** collectionId Identifies the collections to query over.
     * Every collection or subcollection with this ID as the last segment of its
     * path will be included. Cannot contain a slash.
     * @returns {Query***REMOVED*** The created Query.
     *
     * @example
     * let docA = firestore.doc('mygroup/docA').set({foo: 'bar'***REMOVED***);
     * let docB = firestore.doc('abc/def/mygroup/docB').set({foo: 'bar'***REMOVED***);
     *
     * Promise.all([docA, docB]).then(() => {
     *    let query = firestore.collectionGroup('mygroup');
     *    query = query.where('foo', '==', 'bar');
     *    return query.get().then(snapshot => {
     *       console.log(`Found ${snapshot.size***REMOVED*** documents.`);
     *  ***REMOVED***);
     * ***REMOVED***);
     */
    collectionGroup(collectionId) {
        if (collectionId.indexOf('/') !== -1) {
            throw new Error(`Invalid collectionId '${collectionId***REMOVED***'. Collection IDs must not contain '/'.`);
      ***REMOVED***
        return new reference_1.Query(this, reference_1.QueryOptions.forCollectionGroupQuery(collectionId));
  ***REMOVED***
    /**
     * Creates a [WriteBatch]{@link WriteBatch***REMOVED***, used for performing
     * multiple writes as a single atomic operation.
     *
     * @returns {WriteBatch***REMOVED*** A WriteBatch that operates on this Firestore
     * client.
     *
     * @example
     ...
     * let data = { foo: 'bar' ***REMOVED***;
     * writeBatch.set(firestore.doc('col/doc1'), data);
     * writeBatch.set(firestore.doc('col/doc2'), data);
     *
     * writeBatch.commit().then(res => {
     *   console.log(`Added document at ${res.writeResults[0].updateTime***REMOVED***`);
     * ***REMOVED***);
     */
    batch() {
  ...
  ***REMOVED***
     ...
     *
     * @param {function(Transaction)***REMOVED*** updateFunction The function to execute
     * within the transaction context.
     * @param {object=***REMOVED*** transactionOptions Transaction options.
     * @param {number=***REMOVED*** transactionOptions.maxAttempts - The maximum number of
     * attempts for this transaction.
     * @returns {Promise***REMOVED*** If the transaction completed successfully or was
     ...
     *     ***REMOVED***
     *       transaction.update(documentRef, { count: ++count ***REMOVED***);
     ...
     *   console.log(`Count updated to ${res***REMOVED***`);
     * ***REMOVED***);
     */
    runTransaction(updateFunction, transactionOptions) {
        ...
        if (transactionOptions) {
            ...
            validate_1.validateInteger('transactionOptions.maxAttempts', transactionOptions.maxAttempts, { optional: true, minValue: 1 ***REMOVED***);
            attemptsRemaining = transactionOptions.maxAttempts || defaultAttempts;
      ***REMOVED***
        else {
            attemptsRemaining = defaultAttempts;
      ***REMOVED***
        return this.initializeIfNeeded(tag).then(() => this._runTransaction(updateFunction, { requestTag: tag, attemptsRemaining ***REMOVED***));
  ***REMOVED***
      ...
      ***REMOVED***)
            ...
            return transaction
                ...
                if (attemptsRemaining > 1) {
                    logger_1.logger('Firestore.runTransaction', requestTag, `Retrying transaction after error: ${JSON.stringify(err)***REMOVED***.`);
  ...
  ***REMOVED***
     ...
     *     console.log(`Found collection with id: ${collection.id***REMOVED***`);
     * ***REMOVED***
     * ***REMOVED***);
     */
    listCollections() {
  ...
  ***REMOVED***
     ...
     *
     * firestore.getAll(docRef1, docRef2, { fieldMask: ['user'] ***REMOVED***).then(docs => {
     *   console.log(`First document: ${JSON.stringify(docs[0])***REMOVED***`);
     *   console.log(`Second document: ${JSON.stringify(docs[1])***REMOVED***`);
     * ***REMOVED***);
     */
    getAll(...documentRefsOrReadOptions) {
        validate_1.validateMinNumberOfArguments('Firestore.getAll', arguments, 1);
        const { documents, fieldMask ***REMOVED*** = transaction_1.parseGetAllArguments(documentRefsOrReadOptions);
        const tag = util_1.requestTag();
    ...
    getAll_(docRefs, fieldMask, requestTag, transactionId) {
      ...
      ***REMOVED***
        if (fieldMask) {
            ...
            request.mask = { fieldPaths ***REMOVED***;
      ***REMOVED***
        const self = this;
        return self
              ...
              ***REMOVED***)
                    ...
                    for (const docRef of docRefs) {
                      ...
                      ***REMOVED***
                            ...
                            reject(new Error(`Did not receive document for "${docRef.path***REMOVED***".`));
                      ***REMOVED***
                  ***REMOVED***
  ...
  ***REMOVED***
    ...
    createCallOptions() {
        return {
            otherArgs: {
                headers: Object.assign({ [CLOUD_RESOURCE_HEADER]: this.formattedName ***REMOVED***, this._settings.customHeaders),
          ***REMOVED***,
      ***REMOVED***
  ***REMOVED***
    /**
     ...
     * @returns {Promise***REMOVED*** A Promise indicating the function's success.
     */
    /**
 ...
 * @param {string***REMOVED*** Log message
 */
// tslint:disable-next-line:no-default-export
/**
 * The default export of the `@google-cloud/firestore` package is the
 * {@link Firestore***REMOVED*** class.
 *
 * See {@link Firestore***REMOVED*** and {@link ClientConfig***REMOVED*** for client methods and
 * configuration options.
 *
 * @module {Firestore***REMOVED*** @google-cloud/firestore
...
/**
 * {@link v1beta1***REMOVED*** factory function.
 *
 * @private
...
***REMOVED***);
/**
 * {@link v1***REMOVED*** factory function.
 *
 * @private
 * @name Firestore.v1
 * @see v1
