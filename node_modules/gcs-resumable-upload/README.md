...
```
```js
const {upload***REMOVED*** = require('gcs-resumable-upload');
const fs = require('fs');

fs.createReadStream('titanic.mov')
  .pipe(upload({ bucket: 'legally-owned-movies', file: 'titanic.mov' ***REMOVED***))
  .on('finish', () => {
    // Uploaded!
***REMOVED***);
```
...
```js
const {gcsResumableUpload***REMOVED*** = require('gcs-resumable-upload')
...
```js
const {createURI***REMOVED*** = require('gcs-resumable-upload')
````
