...
- [feature] Added `FieldValue.increment()`, which can be used in `update()`
  and `set(..., {merge:true***REMOVED***)` to increment or decrement numeric field
...
- [feature] Access to offline persistence is no longer limited to a single tab.
  ...
  with `{experimentalTabSynchronization: true***REMOVED***`. All tabs accessing persistence
...
# 0.7.0
- [fixed] Fixed `get({source: 'cache'***REMOVED***)` to be able to return nonexistent
...
- [changed] `QuerySnapshot.docChanges()` is now a method that optionally takes
  ...
  with `{ includeMetadataChanges:true ***REMOVED***`, metadata-only document changes are
  suppressed in `docChanges()`.
- [feature] Added new `{ mergeFields: (string|FieldPath)[] ***REMOVED***` option to `set()`
...
- [feature] Added ability to control whether DocumentReference.get() and
  Query.get() should fetch from server only, (by passing { source: 'server' ***REMOVED***),
  cache only (by passing { source: 'cache' ***REMOVED***), or attempt server and fall back
