...
static void enc_tiny(char* buffer) { memcpy(buffer, "1n", 3); ***REMOVED***

/* encode our maximum timeout value, about 1157 days */
static void enc_huge(char* buffer) { memcpy(buffer, "99999999S", 10); ***REMOVED***
...
void grpc_http2_encode_timeout(grpc_millis timeout, char* buffer) {
  ...
  if (timeout <= 0) {
    enc_tiny(buffer);
***REMOVED*** else if (timeout < 1000 * GPR_MS_PER_SEC) {
    enc_millis(buffer, timeout);
***REMOVED*** else if (timeout >= kMaxTimeout) {
    enc_huge(buffer);
***REMOVED*** else {
    enc_seconds(buffer,
