...
/* if changing this, make identical changes in:
   - grpc_core::{InternedMetadata, AllocatedMetadata***REMOVED***
...
#define GRPC_MAKE_MDELEM(data, storage) \
  (grpc_mdelem{((uintptr_t)(data)) | ((uintptr_t)storage)***REMOVED***)
#else
#define GRPC_MAKE_MDELEM(data, storage) \
  ((grpc_mdelem){((uintptr_t)(data)) | ((uintptr_t)storage)***REMOVED***)
#endif
...
class StaticMetadata {
 public:
  StaticMetadata(const grpc_slice& key, const grpc_slice& value, uintptr_t idx)
      : kv_({key, value***REMOVED***), hash_(0), static_idx_(idx) {***REMOVED***

  const grpc_mdelem_data& data() const { return kv_; ***REMOVED***

  void HashInit();
  uint32_t hash() { return hash_; ***REMOVED***
  uintptr_t StaticIndex() { return static_idx_; ***REMOVED***
...
class RefcountedMdBase {
 public:
  RefcountedMdBase(const grpc_slice& key, const grpc_slice& value)
      : key_(key), value_(value), refcnt_(1) {***REMOVED***
  RefcountedMdBase(const grpc_slice& key, const grpc_slice& value,
                   uint32_t hash)
      : key_(key), value_(value), refcnt_(1), hash_(hash) {***REMOVED***

  const grpc_slice& key() const { return key_; ***REMOVED***
  const grpc_slice& value() const { return value_; ***REMOVED***
  uint32_t hash() { return hash_; ***REMOVED***
...
***REMOVED***
 ...
 protected:
  ...
  intptr_t RefValue() { return refcnt_.Load(MemoryOrder::RELAXED); ***REMOVED***
  bool AllRefsDropped() { return refcnt_.Load(MemoryOrder::ACQUIRE) == 0; ***REMOVED***
  bool FirstRef() { return refcnt_.FetchAdd(1, MemoryOrder::RELAXED) == 0; ***REMOVED***
...
class InternedMetadata : public RefcountedMdBase {
 public:
  ...
  struct NoRefKey {***REMOVED***;
  struct BucketLink {
    explicit BucketLink(InternedMetadata* md) : next(md) {***REMOVED***
...
***REMOVED***
  ...
  UserData* user_data() { return &user_data_; ***REMOVED***
  InternedMetadata* bucket_next() { return link_.next; ***REMOVED***
  void set_bucket_next(InternedMetadata* md) { link_.next = md; ***REMOVED***
