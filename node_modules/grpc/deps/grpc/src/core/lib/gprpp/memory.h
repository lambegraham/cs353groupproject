...
***REMOVED***
  ...
  pointer address(reference x) const { return &x; ***REMOVED***
  const_pointer address(const_reference x) const { return &x; ***REMOVED***
  pointer allocate(std::size_t n,
                   std::allocator<void>::const_pointer hint = nullptr) {
    return static_cast<pointer>(gpr_malloc(n * sizeof(T)));
***REMOVED***
  void deallocate(T* p, std::size_t n) { gpr_free(p); ***REMOVED***
  size_t max_size() const {
    return std::numeric_limits<size_type>::max() / sizeof(value_type);
***REMOVED***
  void construct(pointer p, const_reference val) { new ((void*)p) T(val); ***REMOVED***
  template <class U, class... Args>
  void construct(U* p, Args&&... args) {
    ::new ((void*)p) U(std::forward<Args>(args)...);
***REMOVED***
  void destroy(pointer p) { p->~T(); ***REMOVED***
  template <class U>
  void destroy(U* p) {
    p->~U();
***REMOVED***
