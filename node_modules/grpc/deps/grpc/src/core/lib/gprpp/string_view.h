...
class StringView final {
 public:
  ...
  constexpr StringView(const char* ptr, size_t size) : ptr_(ptr), size_(size) {***REMOVED***
  constexpr StringView(const char* ptr)
      : StringView(ptr, ptr == nullptr ? 0 : strlen(ptr)) {***REMOVED***
  ...
  StringView(const grpc_slice& slice)
      : StringView(reinterpret_cast<const char*>(GRPC_SLICE_START_PTR(slice)),
                   GRPC_SLICE_LENGTH(slice)) {***REMOVED***
  constexpr StringView() : StringView(nullptr, 0) {***REMOVED***

  constexpr const char* data() const { return ptr_; ***REMOVED***
  constexpr size_t size() const { return size_; ***REMOVED***
  constexpr bool empty() const { return size_ == 0; ***REMOVED***
...
***REMOVED***
  ...
  constexpr const char& operator[](size_t i) const { return ptr_[i]; ***REMOVED***

  const char& front() const { return ptr_[0]; ***REMOVED***
  const char& back() const { return ptr_[size_ - 1]; ***REMOVED***
...
inline bool operator!=(StringView lhs, StringView rhs) { return !(lhs == rhs); ***REMOVED***
