...
enum requested_call_type { BATCH_CALL, REGISTERED_CALL ***REMOVED***;
...
static void server_ref(grpc_server* server) { server->internal_refcount.Ref(); ***REMOVED***
...
***REMOVED*** else {
    if (gpr_atm_full_cas(&calld->state, NOT_STARTED, ZOMBIED)) {
      GRPC_CLOSURE_INIT(&calld->kill_zombie_closure, kill_zombie, elem,
                        grpc_schedule_on_exec_ctx);
      GRPC_CLOSURE_SCHED(&calld->kill_zombie_closure, GRPC_ERROR_NONE);
  ***REMOVED*** else if (gpr_atm_full_cas(&calld->state, PENDING, ZOMBIED)) {
