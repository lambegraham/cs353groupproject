...
***REMOVED*** else if (fcntl(fd, F_SETFD, FD_CLOEXEC) != 0) {
...
typedef enum { UNKICKED, KICKED, DESIGNATED_POLLER ***REMOVED*** kick_state;
...
static void fd_global_init(void) { gpr_mu_init(&fd_freelist_mu); ***REMOVED***
...
static int fd_wrapped_fd(grpc_fd* fd) { return fd->fd; ***REMOVED***
...
static void fd_become_readable(grpc_fd* fd) { fd->read_closure->SetReady(); ***REMOVED***

static void fd_become_writable(grpc_fd* fd) { fd->write_closure->SetReady(); ***REMOVED***

static void fd_has_errors(grpc_fd* fd) { fd->error_closure->SetReady(); ***REMOVED***
...
typedef enum { EMPTIED, NEW_ROOT, REMOVED ***REMOVED*** worker_remove_result;
...
***REMOVED***
  ...
  ***REMOVED*** else {
    ...
    ***REMOVED***
      ...
      gpr_mu_lock(&pollset->mu);
  ***REMOVED***
***REMOVED*** else if (grpc_core::ExecCtx::Get()->HasWork()) {
    gpr_mu_unlock(&pollset->mu);
    grpc_core::ExecCtx::Get()->Flush();
    gpr_mu_lock(&pollset->mu);
***REMOVED***
...
static grpc_error* pollset_kick(grpc_pollset* pollset,
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_polling_trace)) {
    ...
    if (pollset->root_worker != nullptr) {
      gpr_asprintf(&tmp, " {kick_state=%s next=%p {kick_state=%s***REMOVED******REMOVED***",
  ...
  if (specific_worker == nullptr) {
    ...
    ***REMOVED***
      ...
      ***REMOVED***
        ...
        goto done;
    ***REMOVED*** else if (next_worker->state == KICKED) {
        GRPC_STATS_INC_POLLSET_KICKED_AGAIN();
        if (GRPC_TRACE_FLAG_ENABLED(grpc_polling_trace)) {
    ...
    ***REMOVED*** else if (root_worker ==
      ...
      ***REMOVED***
        ...
        goto done;
    ***REMOVED*** else if (next_worker->state == UNKICKED) {
        GRPC_STATS_INC_POLLSET_KICK_WAKEUP_CV();
        if (GRPC_TRACE_FLAG_ENABLED(grpc_polling_trace)) {
      ...
      ***REMOVED***
        ...
        goto done;
    ***REMOVED*** else if (next_worker->state == DESIGNATED_POLLER) {
...
***REMOVED*** else if (specific_worker ==
  ...
  ***REMOVED***
    ...
    ret_err = grpc_wakeup_fd_wakeup(&global_wakeup_fd);
    goto done;
***REMOVED*** else if (specific_worker->initialized_cv) {
    GRPC_STATS_INC_POLLSET_KICK_WAKEUP_CV();
    if (GRPC_TRACE_FLAG_ENABLED(grpc_polling_trace)) {
...
static void pollset_add_fd(grpc_pollset* pollset, grpc_fd* fd) {***REMOVED***
...
static void pollset_set_destroy(grpc_pollset_set* pss) {***REMOVED***

static void pollset_set_add_fd(grpc_pollset_set* pss, grpc_fd* fd) {***REMOVED***

static void pollset_set_del_fd(grpc_pollset_set* pss, grpc_fd* fd) {***REMOVED***

static void pollset_set_add_pollset(grpc_pollset_set* pss, grpc_pollset* ps) {***REMOVED***

static void pollset_set_del_pollset(grpc_pollset_set* pss, grpc_pollset* ps) {***REMOVED***
...
static bool is_any_background_poller_thread(void) { return false; ***REMOVED***

static void shutdown_background_closure(void) {***REMOVED***
