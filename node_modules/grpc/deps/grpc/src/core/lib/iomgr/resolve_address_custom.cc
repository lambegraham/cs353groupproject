...
static int retry_named_port_failure(grpc_custom_resolver* r,
  ...
  const char* svc[][2] = {{"http", "80"***REMOVED***, {"https", "443"***REMOVED******REMOVED***;
...
void grpc_custom_resolve_callback(grpc_custom_resolver* r,
  ...
  if (error == GRPC_ERROR_NONE) {
    *r->addresses = result;
***REMOVED*** else if (retry_named_port_failure(r, nullptr)) {
    return;
***REMOVED***
  if (r->on_done) {
...
static grpc_address_resolver_vtable custom_resolver_vtable = {
    resolve_address_impl, blocking_resolve_address_impl***REMOVED***;
