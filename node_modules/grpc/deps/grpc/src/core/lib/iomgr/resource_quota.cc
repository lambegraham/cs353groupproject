...
***REMOVED*** while (rq_reclaim_from_per_user_free_pool(resource_quota));
...
static bool rq_alloc(grpc_resource_quota* resource_quota) {
  ...
  ***REMOVED***
    ...
    ***REMOVED***
  ***REMOVED*** else if (GRPC_TRACE_FLAG_ENABLED(grpc_resource_quota_trace) &&
               resource_user->free_pool >= 0) {
...
***REMOVED***
  RuSliceRefcount(grpc_resource_user* resource_user, size_t size)
    ...
    // Nothing to do here.
***REMOVED***
  ~RuSliceRefcount() { grpc_resource_user_free(resource_user_, size_); ***REMOVED***

  grpc_slice_refcount* base_refcount() { return &base_; ***REMOVED***
...
static int rq_cmp(void* a, void* b) { return GPR_ICMP(a, b); ***REMOVED***

const grpc_arg_pointer_vtable* grpc_resource_quota_arg_vtable(void) {
  static const grpc_arg_pointer_vtable vtable = {rq_copy, rq_destroy, rq_cmp***REMOVED***;
  return &vtable;
***REMOVED***
