...
#define GRPC_CALL_COMBINER_STOP(call_combiner, reason) \
 ...
 private:
  ...
  struct TsanLock : public RefCounted<TsanLock, NonPolymorphicRefCount> {
    TsanLock() { TSAN_ANNOTATE_RWLOCK_CREATE(&taken); ***REMOVED***
    ~TsanLock() { TSAN_ANNOTATE_RWLOCK_DESTROY(&taken); ***REMOVED***
    ...
    // true.
    std::atomic<bool> taken{false***REMOVED***;
***REMOVED***
...
class CallCombinerClosureList {
 public:
  CallCombinerClosureList() {***REMOVED***
...
***REMOVED***
  ...
  size_t size() const { return closures_.size(); ***REMOVED***
 ...
 private:
  struct CallCombinerClosure {
    ...
    CallCombinerClosure(grpc_closure* closure, grpc_error* error,
                        const char* reason)
        : closure(closure), error(error), reason(reason) {***REMOVED***
***REMOVED***
