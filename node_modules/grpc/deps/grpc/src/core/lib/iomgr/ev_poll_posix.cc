...
/* Complete polling previously started with fd_begin_poll
   MUST NOT be called with a pollset lock taken
   if got_read or got_write are 1, also does the become_{readable,writable***REMOVED*** as
   appropriate. */
...
***REMOVED*** else if (fd->read_watcher) {
    pollset_kick_locked(fd->read_watcher);
***REMOVED*** else if (fd->write_watcher) {
...
***REMOVED***
#else
static void fd_ref(grpc_fd* fd) { ref_by(fd, 2); ***REMOVED***

static void fd_unref(grpc_fd* fd) { unref_by(fd, 2); ***REMOVED***
#endif
...
***REMOVED*** else if (*st == CLOSURE_NOT_READY) {
    /* not ready ==> switch to a waiting state by setting the closure */
    *st = closure;
***REMOVED*** else if (*st == CLOSURE_READY) {
...
static grpc_error* pollset_kick_ext(grpc_pollset* p,
  ...
  ***REMOVED*** else if ((flags & GRPC_POLLSET_CAN_KICK_SELF) != 0) {
    ...
    ***REMOVED***
      ...
      kick_append_error(&error,
                        grpc_wakeup_fd_wakeup(&specific_worker->wakeup_fd->fd));
  ***REMOVED***
***REMOVED*** else if (gpr_tls_get(&g_current_thread_poller) != (intptr_t)p) {
    GPR_ASSERT((flags & GRPC_POLLSET_REEVALUATE_POLLING_ON_WAKEUP) == 0);
...
static grpc_error* pollset_work(grpc_pollset* pollset,
  ...
  enum { inline_elements = 96 ***REMOVED***;
...
***REMOVED***
  ...
  if (pollset->shutting_down) {
    if (pollset_has_workers(pollset)) {
      pollset_kick(pollset, nullptr);
  ***REMOVED*** else if (!pollset->called_shutdown && !pollset_has_observers(pollset)) {
