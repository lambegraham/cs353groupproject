    ...
    for (unsigned int i = 0; i < channel_args->num_args; i++) {
      if (0 == strcmp(channel_args->args[i].key, GRPC_ARG_KEEPALIVE_TIME_MS)) {
        const int value = grpc_channel_arg_get_integer(
            &channel_args->args[i], grpc_integer_options{0, 1, INT_MAX***REMOVED***);
...
uint16_t grpc_htons(uint16_t hostshort) { return htons(hostshort); ***REMOVED***

uint16_t grpc_ntohs(uint16_t netshort) { return ntohs(netshort); ***REMOVED***

uint32_t grpc_htonl(uint32_t hostlong) { return htonl(hostlong); ***REMOVED***

uint32_t grpc_ntohl(uint32_t netlong) { return ntohl(netlong); ***REMOVED***
