...
typedef enum { PO_MULTI, PO_FD, PO_EMPTY ***REMOVED*** pollable_type;
...
typedef enum { PWLINK_POLLABLE = 0, PWLINK_POLLSET, PWLINK_COUNT ***REMOVED*** pwlinks;
...
static grpc_error* pollable_process_events(grpc_pollset* pollset,
  ...
  if (handle_count == 0) {
    handle_count = 1;
***REMOVED*** else if (handle_count > MAX_EPOLL_EVENTS_HANDLED_EACH_POLL_CALL) {
...
typedef enum { WRR_NEW_ROOT, WRR_EMPTIED, WRR_REMOVED ***REMOVED*** worker_remove_result;
...
static bool begin_worker(grpc_pollset* pollset, grpc_pollset_worker* worker,
  ...
  ***REMOVED***
    while (do_poll && worker->pollable_obj->root_worker != worker) {
      ...
      ***REMOVED***
        do_poll = false;
    ***REMOVED*** else if (worker->kicked) {
        if (GRPC_TRACE_FLAG_ENABLED(grpc_polling_trace)) {
          gpr_log(GPR_INFO, "PS:%p wakeup %p w=%p", pollset,
                  worker->pollable_obj, worker);
      ***REMOVED***
        do_poll = false;
    ***REMOVED*** else if (GRPC_TRACE_FLAG_ENABLED(grpc_polling_trace) &&
                 worker->pollable_obj->root_worker != worker) {
...
#ifndef NDEBUG
static long sys_gettid(void) { return syscall(__NR_gettid); ***REMOVED***
#endif
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    if (a->parent != nullptr) {
      a = a->parent;
  ***REMOVED*** else if (b->parent != nullptr) {
      b = b->parent;
  ***REMOVED*** else {
