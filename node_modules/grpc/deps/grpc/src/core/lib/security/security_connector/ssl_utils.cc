...
grpc_core::RefCountedPtr<grpc_auth_context> grpc_ssl_peer_to_auth_context(
  ...
  ***REMOVED*** else if (strcmp(prop->name,
      ...
      grpc_auth_context_add_property(ctx.get(), GRPC_X509_SAN_PROPERTY_NAME,
                                     prop->value.data, prop->value.length);
  ***REMOVED*** else if (strcmp(prop->name, TSI_X509_PEM_CERT_PROPERTY) == 0) {
      grpc_auth_context_add_property(ctx.get(),
                                     GRPC_X509_PEM_CERT_PROPERTY_NAME,
                                     prop->value.data, prop->value.length);
  ***REMOVED*** else if (strcmp(prop->name, TSI_SSL_SESSION_REUSED_PEER_PROPERTY) == 0) {
      grpc_auth_context_add_property(ctx.get(),
...
tsi_peer grpc_shallow_peer_from_ssl_auth_context(
  ...
  if (max_num_props > 0) {
    ...
    ***REMOVED*** else if (strcmp(prop->name, GRPC_X509_CN_PROPERTY_NAME) == 0) {
        add_shallow_auth_property_to_peer(
            &peer, prop, TSI_X509_SUBJECT_COMMON_NAME_PEER_PROPERTY);
    ***REMOVED*** else if (strcmp(prop->name, GRPC_X509_PEM_CERT_PROPERTY_NAME) == 0) {
