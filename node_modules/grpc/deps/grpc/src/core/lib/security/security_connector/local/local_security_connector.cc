...
void local_check_peer(grpc_security_connector* sc, tsi_peer peer,
  ...
  if (getsockname(fd, reinterpret_cast<grpc_sockaddr*>(resolved_addr.addr),
    ...
    if (type == UDS && grpc_is_unix_socket(addr)) {
      ...
      // IPV4
  ***REMOVED*** else if (type == LOCAL_TCP && sock_addr->sa_family == GRPC_AF_INET) {
    ...
    ***REMOVED***
      // IPv6
  ***REMOVED*** else if (type == LOCAL_TCP && sock_addr->sa_family == GRPC_AF_INET6) {
...
***REMOVED***
   ...
   * {client, server***REMOVED***_auth_filter that verifies if the peer's auth context is
 ...
 public:
  grpc_local_channel_security_connector(
      ...
      : grpc_channel_security_connector(nullptr, std::move(channel_creds),
        ...
        target_name_(gpr_strdup(target_name)) {***REMOVED***

  ~grpc_local_channel_security_connector() override { gpr_free(target_name_); ***REMOVED***
...
***REMOVED***
  ...
  const char* target_name() const { return target_name_; ***REMOVED***
...
class grpc_local_server_security_connector final
 ...
 public:
  grpc_local_server_security_connector(
      ...
      : grpc_server_security_connector(nullptr, std::move(server_creds)) {***REMOVED***
