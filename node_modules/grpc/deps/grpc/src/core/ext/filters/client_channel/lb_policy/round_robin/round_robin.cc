...
class RoundRobin : public LoadBalancingPolicy {
 public:
  ...
  const char* name() const override { return kRoundRobin; ***REMOVED***
  ...
  class RoundRobinSubchannelData
   ...
   public:
    RoundRobinSubchannelData(
        ...
        : SubchannelData(subchannel_list, address, std::move(subchannel)) {***REMOVED***
...
void RoundRobin::RoundRobinSubchannelList::UpdateStateCountersLocked(
  ...
  if (old_state == GRPC_CHANNEL_READY) {
    ...
    --num_ready_;
***REMOVED*** else if (old_state == GRPC_CHANNEL_CONNECTING) {
    GPR_ASSERT(num_connecting_ > 0);
    --num_connecting_;
***REMOVED*** else if (old_state == GRPC_CHANNEL_TRANSIENT_FAILURE) {
    GPR_ASSERT(num_transient_failure_ > 0);
    --num_transient_failure_;
***REMOVED***
  if (new_state == GRPC_CHANNEL_READY) {
    ++num_ready_;
***REMOVED*** else if (new_state == GRPC_CHANNEL_CONNECTING) {
    ++num_connecting_;
***REMOVED*** else if (new_state == GRPC_CHANNEL_TRANSIENT_FAILURE) {
    ++num_transient_failure_;
***REMOVED***
***REMOVED***
...
***REMOVED*** else if (num_connecting_ > 0) {
    /* 2) CONNECTING */
    p->channel_control_helper()->UpdateState(
        GRPC_CHANNEL_CONNECTING, UniquePtr<SubchannelPicker>(New<QueuePicker>(
                                     p->Ref(DEBUG_LOCATION, "QueuePicker"))));
***REMOVED*** else if (num_transient_failure_ == num_subchannels()) {
    /* 3) TRANSIENT_FAILURE */
    grpc_error* error =
...
***REMOVED*** else if (subchannel_list_ == nullptr) {
...
void grpc_lb_policy_round_robin_shutdown() {***REMOVED***
