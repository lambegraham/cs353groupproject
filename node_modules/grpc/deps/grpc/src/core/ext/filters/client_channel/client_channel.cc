...
***REMOVED***
  ...
  bool deadline_checking_enabled() const { return deadline_checking_enabled_; ***REMOVED***
  bool enable_retries() const { return enable_retries_; ***REMOVED***
...
***REMOVED***
  ...
  Mutex* data_plane_mu() const { return &data_plane_mu_; ***REMOVED***
 ...
 private:
  ...
  class ExternalConnectivityWatcher {
   public:
    class WatcherList {
     public:
      WatcherList() { gpr_mu_init(&mu_); ***REMOVED***
      ~WatcherList() { gpr_mu_destroy(&mu_); ***REMOVED***
...
class CallData {
 public:
  ...
  RefCountedPtr<SubchannelCall> subchannel_call() { return subchannel_call_; ***REMOVED***
  ...
  class Metadata : public LoadBalancingPolicy::MetadataInterface {
   public:
    Metadata(CallData* calld, grpc_metadata_batch* batch)
        : calld_(calld), batch_(batch) {***REMOVED***
...
***REMOVED***
  ...
  class LbCallState : public LoadBalancingPolicy::CallState {
   public:
    explicit LbCallState(CallData* calld) : calld_(calld) {***REMOVED***

    void* Alloc(size_t size) override { return calld_->arena_->Alloc(size); ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    ~SubchannelCallBatchData() { Destroy(); ***REMOVED***
    void Destroy();
  ...
  struct SubchannelCallRetryState {
    explicit SubchannelCallRetryState(grpc_call_context_element* context)
        : batch_payload(context),
          ...
          retry_dispatched(false) {***REMOVED***
...
***REMOVED***
  ...
  PendingBatch pending_batches_[MAX_PENDING_BATCHES] = {***REMOVED***;
...
***REMOVED***
  ...
  void AttemptToConnect() override { subchannel_->AttemptToConnect(); ***REMOVED***

  void ResetBackoff() override { subchannel_->ResetBackoff(); ***REMOVED***
...
***REMOVED***
   ...
   public:
    WatcherWrapper(
        ...
        : watcher_(std::move(watcher)),
          ...
          last_seen_state_(initial_state) {***REMOVED***

    ~WatcherWrapper() { parent_.reset(DEBUG_LOCATION, "WatcherWrapper"); ***REMOVED***

    void Orphan() override { Unref(); ***REMOVED***
  ...
  ***REMOVED***
    ...
    grpc_connectivity_state last_seen_state() const { return last_seen_state_; ***REMOVED***
...
***REMOVED***
  ...
  void RequestReresolution() override {***REMOVED***
...
size_t GetMaxPerRpcRetryBufferSize(const grpc_channel_args* args) {
  return static_cast<size_t>(grpc_channel_arg_get_integer(
      ...
      {DEFAULT_PER_RPC_RETRY_BUFFER_SIZE, 0, INT_MAX***REMOVED***));
***REMOVED***
...
bool ChannelData::ProcessResolverResultLocked(
    ...
    if (chand->saved_service_config_ != nullptr) {
      ...
      if (GRPC_TRACE_FLAG_ENABLED(grpc_client_channel_routing_trace)) {
        gpr_log(GPR_INFO,
                ...
                chand);
    ***REMOVED***
  ***REMOVED*** else if (chand->default_service_config_ != nullptr) {
      if (GRPC_TRACE_FLAG_ENABLED(grpc_client_channel_routing_trace)) {
        gpr_log(GPR_INFO,
  ...
  ***REMOVED***
***REMOVED*** else if (result.service_config == nullptr) {
...
CallData::CallData(grpc_call_element* elem, const ChannelData& chand,
    ...
    : deadline_state_(elem, args.call_stack, args.call_combiner,
      ...
      last_attempt_got_server_pushback_(false) {***REMOVED***
...
void CallData::AddRetriableSendInitialMetadataOp(
  ...
  const grpc_slice* retry_count_strings[] = {&GRPC_MDSTR_1, &GRPC_MDSTR_2,
                                             &GRPC_MDSTR_3, &GRPC_MDSTR_4***REMOVED***;
...
void CallData::CreateSubchannelCall(grpc_call_element* elem) {
  ...
  SubchannelCall::Args call_args = {
      ...
      call_context_, call_combiner_, parent_data_size***REMOVED***;
  grpc_error* error = GRPC_ERROR_NONE;
