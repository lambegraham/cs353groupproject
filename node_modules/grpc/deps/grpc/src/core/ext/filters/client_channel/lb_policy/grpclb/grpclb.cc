...
class ParsedGrpcLbConfig : public LoadBalancingPolicy::Config {
 public:
  explicit ParsedGrpcLbConfig(
      ...
      : child_policy_(std::move(child_policy)) {***REMOVED***
  const char* name() const override { return kGrpclb; ***REMOVED***
 ...
 private:
  ...
  class BalancerCallState : public InternallyRefCounted<BalancerCallState> {
   public:
    ...
    GrpcLbClientStats* client_stats() const { return client_stats_.get(); ***REMOVED***

    bool seen_initial_response() const { return seen_initial_response_; ***REMOVED***
    bool seen_serverlist() const { return seen_serverlist_; ***REMOVED***
  ...
  class Serverlist : public RefCounted<Serverlist> {
   public:
    ...
    explicit Serverlist(grpc_grpclb_serverlist* serverlist)
        : serverlist_(serverlist) {***REMOVED***

    ~Serverlist() { grpc_grpclb_destroy_serverlist(serverlist_); ***REMOVED***

    bool operator==(const Serverlist& other) const;

    const grpc_grpclb_serverlist* serverlist() const { return serverlist_; ***REMOVED***
   ...
   public:
    Picker(GrpcLb* parent, RefCountedPtr<Serverlist> serverlist,
        ...
        : parent_(parent),
          ...
          client_stats_(std::move(client_stats)) {***REMOVED***
...
***REMOVED***
void lb_token_destroy(void* token) { gpr_free(token); ***REMOVED***
...
const grpc_arg_pointer_vtable lb_token_arg_vtable = {
    lb_token_copy, lb_token_destroy, equal_cmp***REMOVED***;
const grpc_arg_pointer_vtable client_stats_arg_vtable = {
    client_stats_copy, client_stats_destroy, equal_cmp***REMOVED***;
    ...
    if (client_stats_report_interval != nullptr) {
      ...
      if (GRPC_TRACE_FLAG_ENABLED(grpc_lb_glb_trace)) {
        gpr_log(GPR_INFO,
                ...
                lb_calld->client_stats_report_interval_);
    ***REMOVED***
  ***REMOVED*** else if (GRPC_TRACE_FLAG_ENABLED(grpc_lb_glb_trace)) {
      gpr_log(GPR_INFO,
...
***REMOVED*** else if ((serverlist = grpc_grpclb_response_parse_serverlist(
...
***REMOVED***
  ...
  lb_call_timeout_ms_ = grpc_channel_arg_get_integer(arg, {0, 0, INT_MAX***REMOVED***);
  // Record fallback-at-startup timeout.
  arg = grpc_channel_args_find(args.args, GRPC_ARG_GRPCLB_FALLBACK_TIMEOUT_MS);
  fallback_at_startup_timeout_ = grpc_channel_arg_get_integer(
      arg, {GRPC_GRPCLB_DEFAULT_FALLBACK_TIMEOUT_MS, 0, INT_MAX***REMOVED***);
***REMOVED***
...
void grpc_lb_policy_grpclb_shutdown() {***REMOVED***
