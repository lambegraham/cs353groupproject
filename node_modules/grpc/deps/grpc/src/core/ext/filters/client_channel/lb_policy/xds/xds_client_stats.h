...
***REMOVED***
  ...
  XdsLocalityName(UniquePtr<char> region, UniquePtr<char> zone,
      ...
      : region_(std::move(region)),
        zone_(std::move(zone)),
        sub_zone_(std::move(subzone)) {***REMOVED***
...
***REMOVED***
  ...
  const char* region() const { return region_.get(); ***REMOVED***
  const char* zone() const { return zone_.get(); ***REMOVED***
  const char* sub_zone() const { return sub_zone_.get(); ***REMOVED***

  const char* AsHumanReadableString() {
    if (human_readable_string_ == nullptr) {
      char* tmp;
      gpr_asprintf(&tmp, "{region=\"%s\", zone=\"%s\", sub_zone=\"%s\"***REMOVED***",
...
class XdsClientStats {
 public:
  class LocalityStats : public RefCounted<LocalityStats> {
    ...
    ***REMOVED***
     ...
     private:
      uint64_t num_requests_finished_with_metric_{0***REMOVED***;
      double total_metric_value_{0***REMOVED***;
  ***REMOVED***
  ...
  ***REMOVED***
    ...
    void RefByPicker() { picker_refcount_.FetchAdd(1, MemoryOrder::ACQ_REL); ***REMOVED***
    ...
    void UnrefByPicker() { picker_refcount_.FetchSub(1, MemoryOrder::ACQ_REL); ***REMOVED***
    // Only be called from the control plane combiner.
  ...
  ***REMOVED***
   ...
   private:
    Atomic<uint64_t> total_successful_requests_{0***REMOVED***;
    Atomic<uint64_t> total_requests_in_progress_{0***REMOVED***;
    // Requests that were issued (not dropped) but failed.
    Atomic<uint64_t> total_error_requests_{0***REMOVED***;
    Atomic<uint64_t> total_issued_requests_{0***REMOVED***;
    ...
    // combiner.
    Atomic<uint8_t> picker_refcount_{0***REMOVED***;
***REMOVED***
...
***REMOVED***
 ...
 private:
  ...
  Atomic<uint64_t> total_dropped_requests_{0***REMOVED***;
