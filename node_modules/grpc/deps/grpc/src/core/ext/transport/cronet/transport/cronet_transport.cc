...
***REMOVED***
  ...
  int length_field = 0;
  bool compressed = 0;
  char grpc_header_bytes[GRPC_HEADER_SIZE_IN_BYTES] = {***REMOVED***;
...
struct op_state {
  op_state(grpc_core::Arena* arena) : rs(arena) {***REMOVED***

  bool state_op_done[OP_NUM_OPS] = {***REMOVED***;
  bool state_callback_received[OP_NUM_OPS] = {***REMOVED***;
...
void grpc_cronet_stream_ref(stream_obj* s) { grpc_stream_ref(s->refcount); ***REMOVED***
void grpc_cronet_stream_unref(stream_obj* s) { grpc_stream_unref(s->refcount); ***REMOVED***
#endif
...
inline op_and_state::op_and_state(stream_obj* s,
    ...
    : op(op), state(s->arena), s(s) {***REMOVED***
...
***REMOVED*** else {
    for (curr = s->storage.head; curr != nullptr; curr = curr->next) {
      if (curr->next == oas) {
        ...
        grpc_core::Delete(oas);
        break;
    ***REMOVED*** else if (GPR_UNLIKELY(curr->next == nullptr)) {
...
static void execute_from_storage(stream_obj* s) {
  ...
  for (struct op_and_state* curr = s->storage.head; curr != nullptr;) {
    ...
    if (curr->done) {
      ...
      curr = next;
  ***REMOVED*** else if (result == NO_ACTION_POSSIBLE) {
      curr = curr->next;
  ***REMOVED*** else if (result == ACTION_TAKEN_WITH_CALLBACK) {
      /* wait for the callback */
      break;
  ***REMOVED*** /* continue processing the same op if ACTION_TAKEN_WITHOUT_CALLBACK */
***REMOVED***
  gpr_mu_unlock(&s->mu);
***REMOVED***
...
static bool op_can_be_run(grpc_transport_stream_op_batch* curr_op,
  ...
  ***REMOVED***
    ...
    if (op_id == OP_ON_COMPLETE &&
      ...
      result = false;
  ***REMOVED***
***REMOVED*** else if (op_id == OP_SEND_INITIAL_METADATA) {
    /* already executed */
    if (stream_state->state_op_done[OP_SEND_INITIAL_METADATA]) result = false;
***REMOVED*** else if (op_id == OP_RECV_INITIAL_METADATA) {
    /* already executed */
    ...
    /* we haven't received headers yet. */
             ...
             !stream_state->state_op_done[OP_RECV_TRAILING_METADATA])
      result = false;
***REMOVED*** else if (op_id == OP_SEND_MESSAGE) {
    ...
    /* we haven't sent headers yet. */
      ...
      result = false;
***REMOVED*** else if (op_id == OP_RECV_MESSAGE) {
    /* already executed */
    ...
    /* we haven't received headers yet. */
             ...
             !stream_state->state_op_done[OP_RECV_TRAILING_METADATA])
      result = false;
***REMOVED*** else if (op_id == OP_RECV_TRAILING_METADATA) {
    /* already executed */
    ...
    /* we haven't received on_succeeded  yet. */
      ...
      result = false;
***REMOVED*** else if (op_id == OP_SEND_TRAILING_METADATA) {
    /* already executed */
    ...
    /* we haven't got on_write_completed for the send yet */
      ...
      result = false;
***REMOVED*** else if (op_id == OP_CANCEL_ERROR) {
    /* already executed */
    if (stream_state->state_op_done[OP_CANCEL_ERROR]) result = false;
***REMOVED*** else if (op_id == OP_ON_COMPLETE) {
    /* already executed (note we're checking op specific state, not stream
    state) */
  ...
  ***REMOVED*** else if (curr_op->cancel_stream &&
      ...
      CRONET_LOG(GPR_DEBUG, "Because");
      result = false;
  ***REMOVED*** else if (curr_op->recv_trailing_metadata) {
...
***REMOVED*** else if (stream_op->recv_initial_metadata &&
    ...
    if (stream_state->state_op_done[OP_CANCEL_ERROR]) {
      GRPC_CLOSURE_SCHED(
          ...
          GRPC_ERROR_NONE);
  ***REMOVED*** else if (stream_state->state_callback_received[OP_FAILED]) {
      GRPC_CLOSURE_SCHED(
          stream_op->payload->recv_initial_metadata.recv_initial_metadata_ready,
          GRPC_ERROR_NONE);
  ***REMOVED*** else if (stream_state->state_op_done[OP_RECV_TRAILING_METADATA]) {
      GRPC_CLOSURE_SCHED(
          stream_op->payload->recv_initial_metadata.recv_initial_metadata_ready,
          GRPC_ERROR_NONE);
  ***REMOVED*** else {
...
***REMOVED*** else if (stream_op->recv_message &&
  ...
  ***REMOVED*** else if (stream_state->state_callback_received[OP_FAILED]) {
      ...
      oas->state.state_op_done[OP_RECV_MESSAGE] = true;
      result = ACTION_TAKEN_NO_CALLBACK;
  ***REMOVED*** else if (stream_state->rs.read_stream_closed == true) {
      ...
      oas->state.state_op_done[OP_RECV_MESSAGE] = true;
      result = ACTION_TAKEN_NO_CALLBACK;
  ***REMOVED*** else if (stream_state->flush_read) {
      ...
      oas->state.state_op_done[OP_RECV_MESSAGE] = true;
      result = ACTION_TAKEN_NO_CALLBACK;
  ***REMOVED*** else if (stream_state->rs.length_field_received == false) {
      ...
      ***REMOVED*** else {
        ...
        ***REMOVED***
          ...
          result = ACTION_TAKEN_NO_CALLBACK;
      ***REMOVED***
    ***REMOVED*** else if (stream_state->rs.remaining_bytes == 0) {
...
***REMOVED*** else if (stream_op->recv_trailing_metadata &&
  ...
  ***REMOVED*** else if (oas->s->state.rs.trailing_metadata_valid) {
      grpc_chttp2_incoming_metadata_buffer_publish(
...
static void destroy_transport(grpc_transport* gt) {***REMOVED***

static grpc_endpoint* get_endpoint(grpc_transport* gt) { return nullptr; ***REMOVED***

static void perform_op(grpc_transport* gt, grpc_transport_op* op) {***REMOVED***
