...
grpc_error* grpc_chttp2_perform_read(grpc_chttp2_transport* t,
  ...
  switch (t->deframe_state) {
    ...
    ***REMOVED***
      ...
      ***REMOVED***
        goto dts_fh_0; /* loop */
    ***REMOVED*** else if (t->flow_control->flow_control_enabled() &&
    ...
    ***REMOVED***
    ...
    case GRPC_DTS_FRAME:
      ...
      ***REMOVED***
        ...
        t->incoming_stream = nullptr;
        return GRPC_ERROR_NONE;
    ***REMOVED*** else if (static_cast<uint32_t>(end - cur) > t->incoming_frame_size) {
...
error_handler:
  ...
  if (err == GRPC_ERROR_NONE) {
    ...
    return GRPC_ERROR_NONE;
***REMOVED*** else if (grpc_error_get_int(err, GRPC_ERROR_INT_STREAM_ID, &unused)) {
...
static void free_timeout(void* p) { gpr_free(p); ***REMOVED***
...
***REMOVED***
  ...
  if (is_nonzero_status(md)) {  // not GRPC_MDELEM_GRPC_STATUS_0?
    s->seen_error = true;
***REMOVED*** else if (md_key_cmp(md, GRPC_MDSTR_GRPC_TIMEOUT)) {
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    ***REMOVED***
      return err;
  ***REMOVED*** else if (GPR_UNLIKELY(t->last_new_stream_id >= t->incoming_stream_id)) {
      GRPC_CHTTP2_IF_TRACING(gpr_log(
          GPR_ERROR,
      ...
      return init_skip_frame_parser(t, 1);
  ***REMOVED*** else if (GPR_UNLIKELY((t->incoming_stream_id & 1) == 0)) {
      GRPC_CHTTP2_IF_TRACING(gpr_log(
          GPR_ERROR,
