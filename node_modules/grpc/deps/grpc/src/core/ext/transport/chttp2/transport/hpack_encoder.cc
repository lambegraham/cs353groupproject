...
static const grpc_slice terminal_slice = {
    ...
    {{0, nullptr***REMOVED******REMOVED***            /* data.refcounted */
***REMOVED***;
...
***REMOVED***
  ...
  /* Store this element into {entries,indices***REMOVED***_elem */
  if (grpc_mdelem_both_interned_eq(c->entries_elems[HASH_FRAGMENT_2(elem_hash)],
                                   elem)) {
    /* already there: update with new index */
    c->indices_elems[HASH_FRAGMENT_2(elem_hash)] = new_index;
***REMOVED*** else if (grpc_mdelem_both_interned_eq(
                 c->entries_elems[HASH_FRAGMENT_3(elem_hash)], elem)) {
    /* already there (cuckoo): update with new index */
    c->indices_elems[HASH_FRAGMENT_3(elem_hash)] = new_index;
***REMOVED*** else if (GRPC_MDISNULL(c->entries_elems[HASH_FRAGMENT_2(elem_hash)])) {
    /* not there, but a free element: add */
    c->entries_elems[HASH_FRAGMENT_2(elem_hash)] = GRPC_MDELEM_REF(elem);
    c->indices_elems[HASH_FRAGMENT_2(elem_hash)] = new_index;
***REMOVED*** else if (GRPC_MDISNULL(c->entries_elems[HASH_FRAGMENT_3(elem_hash)])) {
  ...
  wire_value(uint8_t huffman_prefix, bool insert_null_before_wire_value,
      ...
      : data(slice),
        ...
        length(GRPC_SLICE_LENGTH(slice) +
               (insert_null_before_wire_value ? 1 : 0)) {***REMOVED***
...
namespace {
enum class EmitLitHdrType { INC_IDX, NO_IDX ***REMOVED***;

enum class EmitLitHdrVType { INC_IDX_V, NO_IDX_V ***REMOVED***;
***REMOVED***  // namespace
...
struct EmitIndexedStatus {
  ...
  EmitIndexedStatus(uint32_t elem_hash, bool emitted, bool can_add)
      : elem_hash(elem_hash), emitted(emitted), can_add(can_add) {***REMOVED***
...
***REMOVED***
  if (should_add_elem) {
    add_elem(c, elem, decoder_space_usage, elem_hash, key_hash);
***REMOVED*** else if (should_add_key) {
...
static uint32_t elems_for_bytes(uint32_t bytes) { return (bytes + 31) / 32; ***REMOVED***
...
***REMOVED*** else if (c->max_table_elems < c->cap_table_elems / 3) {
