...
bool is_pow2(uint64_t v) { return v == 0 || (v & (v - 1)) == 0; ***REMOVED***
...
static upb_tabent *emptyent(upb_table *t) {
  ...
  while (1) { if (upb_tabent_isempty(--e)) return e; UPB_ASSERT(e > t->entries); ***REMOVED***
***REMOVED***
...
static size_t next(const upb_table *t, size_t i) {
  do {
    if (++i >= upb_table_size(t))
      return SIZE_MAX;
***REMOVED*** while(upb_tabent_isempty(&t->entries[i]));
...
static uint32_t inthash(upb_tabkey key) { return upb_inthash(key); ***REMOVED***
...
void upb_inttable_compact2(upb_inttable *t, upb_alloc *a) {
  ...
  size_t counts[UPB_MAXARRSIZE + 1] = {0***REMOVED***;

  /* The max key in each bucket. */
  uintptr_t max[UPB_MAXARRSIZE + 1] = {0***REMOVED***;
...
***REMOVED***
  ...
  for (size_lg2 = ARRAY_SIZE(counts) - 1; size_lg2 > 0; size_lg2--) {
    if (counts[size_lg2] == 0) {
      ...
      continue;
  ***REMOVED*** else if (arr_count >= (1 << size_lg2) * MIN_DENSITY) {
      break;
  ***REMOVED***
...
#define MIX(h,k,m) { k *= m; k ^= k >> r; k *= m; h *= m; h ^= k; ***REMOVED***
