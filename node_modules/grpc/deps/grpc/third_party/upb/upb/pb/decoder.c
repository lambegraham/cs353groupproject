...
***REMOVED*** else if (bufleft(d) >= bytes) {
...
***REMOVED***
  if (curbufleft(d) >= bytes) {
    ...
    return DECODE_OK;
***REMOVED*** else if (d->data_end == d->delim_end) {
...
UPB_FORCEINLINE static int32_t decode_varint(upb_pbdecoder *d, uint64_t *u64) {
  if (curbufleft(d) > 0 && !(*d->ptr & 0x80)) {
    *u64 = *d->ptr;
    advance(d, 1);
    return DECODE_OK;
***REMOVED*** else if (curbufleft(d) >= 10) {
    /* Fast case. */
...
static double as_double(uint64_t n) { double d; memcpy(&d, &n, 8); return d; ***REMOVED***
static float  as_float(uint32_t n)  { float  f; memcpy(&f, &n, 4); return f; ***REMOVED***
...
static void decoder_pop(upb_pbdecoder *d) { d->top--; ***REMOVED***
...
UPB_NOINLINE int32_t upb_pbdecoder_checktag_slow(upb_pbdecoder *d,
  ...
  if (read == bytes && data == expected) {
    ...
    UPB_ASSERT(ok < 0);
    return DECODE_OK;
***REMOVED*** else if (read < bytes && memcmp(&data, &expected, read) == 0) {
    return suspend_save(d);
***REMOVED*** else {
    ...
    ***REMOVED***
      ...
      case UPB_WIRE_TYPE_END_GROUP:
        if (fieldnum == -d->top->groupnum) {
          decoder_pop(d);
      ***REMOVED*** else if (fieldnum == d->top->groupnum) {
...
static int32_t dispatch(upb_pbdecoder *d) {
  ...
  if (fieldnum != DISPATCH_ENDMSG &&
    ...
    if (wire_type == (v & 0xff)) {
      ...
      return DECODE_OK;
  ***REMOVED*** else if (wire_type == ((v >> 8) & 0xff)) {
      bool found =
...
#define VMCASE(op, code) \
  case op: { code; if (consumes_input(op)) checkpoint(d); break; ***REMOVED***
