...
#define CHK(x) do { if (!(x)) { return false; ***REMOVED*** ***REMOVED*** while(0)

static size_t upb_encode_varint(uint64_t val, char *buf) {
  size_t i;
  if (val < 128) { buf[0] = val; return 1; ***REMOVED***
  i = 0;
  while (val) {
...
static uint32_t upb_zzencode_32(int32_t n) { return ((uint32_t)n << 1) ^ (n >> 31); ***REMOVED***
static uint64_t upb_zzencode_64(int64_t n) { return ((uint64_t)n << 1) ^ (n >> 63); ***REMOVED***

typedef struct {
  upb_alloc *alloc;
  char *buf, *ptr, *limit;
***REMOVED*** upb_encstate;
