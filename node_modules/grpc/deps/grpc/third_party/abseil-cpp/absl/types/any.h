 ...
 public:
  ...
  any(const any& other)
      : obj_(other.has_value() ? other.obj_->Clone()
                               : std::unique_ptr<ObjInterface>()) {***REMOVED***
  ...
  any(T&& value) : obj_(new Obj<VT>(in_place, std::forward<T>(value))) {***REMOVED***
  ...
  explicit any(in_place_type_t<T> /*tag*/, Args&&... args)
      : obj_(new Obj<VT>(in_place, std::forward<Args>(args)...)) {***REMOVED***
  ...
  explicit any(in_place_type_t<T> /*tag*/, std::initializer_list<U> ilist,
               Args&&... args)
      : obj_(new Obj<VT>(in_place, ilist, std::forward<Args>(args)...)) {***REMOVED***
...
***REMOVED***
  ...
  // if present.
  void reset() noexcept { obj_ = nullptr; ***REMOVED***

  // any::swap()
  //
  // Swaps the passed value and the value of this `absl::any` object.
  void swap(any& other) noexcept { obj_.swap(other.obj_); ***REMOVED***
  ...
  // returns `false`.
  bool has_value() const noexcept { return obj_ != nullptr; ***REMOVED***
  ...
  class Obj : public ObjInterface {
   public:
    ...
    explicit Obj(in_place_t /*tag*/, Args&&... args)
        : value(std::forward<Args>(args)...) {***REMOVED***

    std::unique_ptr<ObjInterface> Clone() const final {
      return std::unique_ptr<ObjInterface>(new Obj(in_place, value));
  ***REMOVED***

    const void* ObjTypeId() const noexcept final { return IdForType<T>(); ***REMOVED***

#if ABSL_ANY_DETAIL_HAS_RTTI
    const std::type_info& Type() const noexcept final { return typeid(T); ***REMOVED***
#endif  // ABSL_ANY_DETAIL_HAS_RTTI
...
template <typename T>
struct any::IsInPlaceType : std::false_type {***REMOVED***;

template <typename T>
struct any::IsInPlaceType<in_place_type_t<T>> : std::true_type {***REMOVED***;

inline void swap(any& x, any& y) noexcept { x.swap(y); ***REMOVED***
