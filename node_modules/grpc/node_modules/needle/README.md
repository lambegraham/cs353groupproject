...
needle
  .post('https://my.server.com/foo', data, { multipart: true ***REMOVED***)
  .on('readable', function() { /* eat your chunks */ ***REMOVED***)
...
```js
needle('put', 'https://hacking.the.gibson/login', { password: 'god' ***REMOVED***, { json: true ***REMOVED***)
...
```js
needle.get('http://stackoverflow.com/feeds', { compressed: true ***REMOVED***, function(err, resp) {
...
needle('get', 'http://some.url.com')
  .then(function(resp) { console.log(resp.body) ***REMOVED***)
  .catch(function(err) { console.error(err) ***REMOVED***)
***REMOVED***)
```
...
```js
var options = {
  headers: { 'X-Custom-Header': 'Bumbaway atuna' ***REMOVED***
***REMOVED***
...
```js
needle.request('get', 'forum.com/search', params, { json: true ***REMOVED***, function(err, resp) {
...
These are basically shortcuts to the `headers` option described above.
 ...
 - `cookies`   : Builds and sets a Cookie header from a `{ key: 'value' ***REMOVED***` object.
 ...
 - `user_agent`: Sets the 'User-Agent' HTTP header. Defaults to `Needle/{version***REMOVED*** (Node.js {node_version***REMOVED***)`.
...
var myAgent = tunnel.httpOverHttp({
  proxy: { host: 'localhost' ***REMOVED***
***REMOVED***);

needle.get('foobar.com', { agent: myAgent ***REMOVED***);
```
...
So if you're stuck on 0.10 or even lower and want full speed, you can simply set the Connection header to 'Keep-Alive' by using `{ connection: 'Keep-Alive' ***REMOVED***`. Please note, though, that an event loop handler will prevent the runtime from exiting so you'll need to manually call `process.exit()` or the universe will collapse.
...
```js
needle.get('https://api.server.com', { username: 'you', password: 'secret' ***REMOVED***,
  function(err, resp) {
    // used HTTP auth
***REMOVED***);
```
...
```js
needle.get('other.server.com', { username: 'you', password: 'secret', auth: 'digest' ***REMOVED***,
...
```js
needle.get('http://upload.server.com/tux.png', { output: '/tmp/tux.png' ***REMOVED***, function(err, resp, body) {
...
```js
needle.get('http://search.npmjs.org', { proxy: 'http://localhost:1234' ***REMOVED***, function(err, resp, body) {
...
```js
var stream = needle.get('http://jsonplaceholder.typicode.com/db', { parse: true ***REMOVED***);
...
needle.get('http://jsonplaceholder.typicode.com/db', { parse: true ***REMOVED***)
...
var data = {
  foo: 'bar',
  image: { file: '/home/tomas/linux.png', content_type: 'image/png' ***REMOVED***
***REMOVED***

needle.post('http://my.other.app.com', data, { multipart: true ***REMOVED***, function(err, resp, body) {
...
needle.post('http://somewhere.com/over/the/rainbow', data, { multipart: true ***REMOVED***, function(err, resp, body) {
...
var data = {
  token: 'verysecret',
  payload: {
    value: JSON.stringify({ title: 'test', version: 1 ***REMOVED***),
    content_type: 'application/json'
***REMOVED***
***REMOVED***

needle.post('http://test.com/', data, { timeout: 5000, multipart: true ***REMOVED***, function(err, resp, body) {
