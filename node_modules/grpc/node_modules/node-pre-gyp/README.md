...
It **is recommended** that you customize this property. This is an extra path to use for publishing and finding remote tarballs. The default value for `remote_path` is `""` meaning that if you do not provide it then all packages will be published at the base of the `host`. It is recommended to provide a value like `./{name***REMOVED***/v{version***REMOVED***` to help organize remote packages in the case that you choose to publish multiple node addons to the same `host`.

Note: This property supports variables based on [Versioning](#versioning).

###### package_name

It is **not recommended** to override this property unless you are also overriding the `remote_path`. This is the versioned name of the remote tarball containing the binary `.node` module and any supporting files you've placed inside the `module_path` directory. Unless you specify `package_name` in your `package.json` then it defaults to `{module_name***REMOVED***-v{version***REMOVED***-{node_abi***REMOVED***-{platform***REMOVED***-{arch***REMOVED***.tar.gz` which allows your binary to work across node versions, platforms, and architectures. If you are using `remote_path` that is also versioned by `./{module_name***REMOVED***/v{version***REMOVED***` then you could remove these variables from the `package_name` and just use: `{node_abi***REMOVED***-{platform***REMOVED***-{arch***REMOVED***.tar.gz`. Then your remote tarball will be looked up at, for example, `https://example.com/your-module/v0.1.0/node-v11-linux-x64.tar.gz`.
...
Specifically, when performing N-API builds, the `{napi_build_version***REMOVED***` text configuration value  *must* be present in the `module_path` property. In addition, the `{napi_build_version***REMOVED***` text configuration value  *must* be present in either the `remote_path` or `package_name` property. (No problem if it's in both.)
...
"binary": {
    "module_name": "your_module",
    "module_path": "./lib/binding/napi-v{napi_build_version***REMOVED***",
    "remote_path": "./{module_name***REMOVED***/v{version***REMOVED***/{configuration***REMOVED***/",
    "package_name": "{platform***REMOVED***-{arch***REMOVED***-napi-v{napi_build_version***REMOVED***.tar.gz",
    "host": "https://your_bucket.s3-us-west-1.amazonaws.com",
    "napi_versions": [1,3]
***REMOVED***
```
...
"binary": {
    "module_name": "your_module",
    "module_path": "./lib/binding/{node_napi_label***REMOVED***",
    "remote_path": "./{module_name***REMOVED***/v{version***REMOVED***/{configuration***REMOVED***/",
    "package_name": "{platform***REMOVED***-{arch***REMOVED***-{node_napi_label***REMOVED***.tar.gz",
    "host": "https://your_bucket.s3-us-west-1.amazonaws.com",
    "napi_versions": [1,3]
***REMOVED***
```
...
These values are present for use in the `binding.gyp` file and may be used as `{napi_version***REMOVED***` and `{node_abi_napi***REMOVED***` for text substituion in the `binary` properties of the `package.json` file.
...
If your commit message contains special characters (e.g. `&`) this method might fail. An alternative is to use PowerShell, which gives you additional possibilities, like ignoring case by using `ToLower()`:
    ...
    ps: if($env:APPVEYOR_REPO_COMMIT_MESSAGE.ToLower().Contains('[publish binary]')) { node-pre-gyp --msvs_version=2013 publish ***REMOVED***
...
Use `travis-encrypt` like:
    ...
    travis encrypt node_pre_gyp_accessKeyId=${node_pre_gyp_accessKeyId***REMOVED***
    travis encrypt node_pre_gyp_secretAccessKey=${node_pre_gyp_secretAccessKey***REMOVED***
...
```sh
export PATH=$(pwd)/node_modules/.bin:${PATH***REMOVED***
```
...
You might wish to publish binaries only on a specific commit. To do this you could borrow from the [Travis CI idea of commit keywords](http://about.travis-ci.org/docs/user/how-to-skip-a-build/) and add special handling for commit messages with `[publish binary]`:
    ...
    if [[ ${COMMIT_MESSAGE***REMOVED*** =~ "[publish binary]" ]]; then node-pre-gyp publish; fi;
...
Using the `npm` config argument: `--{module_name***REMOVED***_binary_host_mirror` can download binary files through a mirror.
