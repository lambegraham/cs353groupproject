...
).then(_ => { .. tarball has been created .. ***REMOVED***)
```
...
tar.x(  // or tar.extract(
  {
    file: 'my-tarball.tgz'
***REMOVED***
).then(_=> { .. tarball has been dumped in cwd .. ***REMOVED***)
```
...
tar.t({
  file: 'my-tarball.tgz',
  onentry: entry => { .. do whatever with it .. ***REMOVED***
***REMOVED***)
```
...
fs.createReadStream('my-tarball.tgz')
  .pipe(tar.t())
  .on('entry', entry => { .. do whatever with it .. ***REMOVED***)
```
