...
```html
<script src="https://www.gstatic.com/firebasejs/ui/4.2.0/firebase-ui-auth__{LANGUAGE_CODE***REMOVED***.js"></script>
<link type="text/css" rel="stylesheet" href="https://www.gstatic.com/firebasejs/ui/4.2.0/firebase-ui-auth.css" />
```

where `{LANGUAGE_CODE***REMOVED***` is replaced by the code of the language you want. For example, the French
...
<td>
  ...
  specific events.
  <br/>
  <em>Default:</em> <code>{***REMOVED***</code>
</td>
</tr>
<tr>
...
```bash
npm run build build-js-{LANGUAGE_CODE***REMOVED***
```
where `{LANGUAGE_CODE***REMOVED***` is replaced by the
...
```bash
npm run build build-npm-{LANGUAGE_CODE***REMOVED***
```
Make sure all underscore symbols in the `LANGUAGE_CODE` are replaced with
dashes.
This will generate `dist/npm__{LANGUAGE_CODE***REMOVED***.js`.
You can then import/require it:
```javascript
import firebaseui from './npm__{LANGUAGE_CODE***REMOVED***';
```
